...
static void set_initial_user_modes(struct Client *cptr) {
  char **umodev;
  int i, num, l, offset;
  char *tmpstr = client_get_default_umode(cptr);
  char *back;
  ...
  if(num + offset > 0)
    set_user_mode(&me, cptr, 2 + num + offset, umodev, ALLOWMODES_ANY);
  MyFree(umodev);
  MyFree(tmpstr);
}
...
int register_user(struct Client *cptr, struct Client *sptr)
{
  ...
  if (MyUser(sptr))
  {
    ...
    if(HasPriv(sptr, PRIV_CHAN_LIMIT))
      ...
      sendcmdto_one(&me, CMD_NOTICE, sptr, "%C :You may join %d channels.", sptr, cli_confs(sptr)->value.aconf ?
                                                                                  ConfMaxChannels(cli_confs(sptr)->value.aconf):
 ...
 * nothing is done.
 */
void apply_fakehost(struct Client *cptr) {
    if(IsFakeHost(cptr)) {
        ircd_strncpy(cli_user(cptr)->host, cli_user(cptr)->fakehost, HOSTLEN);
        return;
    }
    if (IsAccount(cptr)) {
        ircd_snprintf(0, cli_user(cptr)->host, HOSTLEN, "%s.%s", cli_user(cptr)->account, feature_str(FEAT_HIDDEN_HOST));
        return;
    }
}
...
hide_hostmask(struct Client *cptr, unsigned int flag)
{
  ...
  for (chan = cli_user(cptr)->channel; chan; chan = chan->next_channel)
  {
    ...
    if(!(chan->channel->mode.mode & MODE_DELJOINS) || IsChanOp(chan) || HasVoice(chan)) {
		...
		char *parv[], int allow_modes)
{
  ...
  if (!IsServer(cptr) && !IsMe(cptr))
  {
    ...
    if (!FlagHas(&setflags, FLAG_XTRAOP) && IsXtraOp(sptr) && (!IsOper(sptr) || !HasPriv(sptr, PRIV_UMODE_XTRAOP)))
      ClearXtraOp(sptr);
    if (!FlagHas(&setflags, FLAG_NETSERV) && IsNetServ(sptr) && !HasPriv(sptr, PRIV_UMODE_NETSERV))
      ClearNetServ(sptr);
